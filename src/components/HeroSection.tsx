      ...
      <motion.div className="absolute inset-0" style={{ y: bgY }}>
        ...
        {STARS.map((star) => (
          <motion.div
            ...
            animate={{ opacity: [star.opacity, star.opacity * 3, star.opacity], scale: [1, 1.5, 1] }}
        ...
        {SHOOTING_STARS.map((s) => (
          <motion.div
            ...
            style={{ top: `${s.top}%`, left: `${s.left}%`, width: "120px", background: "linear-gradient(to right, transparent, rgba(255,120,50,0.8), transparent)", rotate: "-35deg" }}
            animate={{ x: [0, 300], y: [0, 150], opacity: [0, 1, 0] }}
            transition={{ duration: s.duration, repeat: Infinity, delay: s.delay, ease: "easeIn" }}
          />
        ))}
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <div className="text-center md:text-left order-2 md:order-1">
            <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
              <span className="text-lg md:text-xl text-muted-foreground font-light tracking-wide">
                {greeting}
                <motion.span animate={{ opacity: [1, 0] }} transition={{ duration: 0.5, repeat: Infinity, repeatType: "reverse" }} className={greetingDone ? "hidden" : "inline-block w-[2px] h-5 bg-primary ml-1 align-middle"} />
              </span>
            </motion.div>

            <motion.h1 initial={{ opacity: 0 }} animate={{ opacity: greetingDone ? 1 : 0 }} transition={{ duration: 0.3 }} className="text-4xl md:text-6xl lg:text-7xl font-display font-bold my-4 leading-tight">
              <span className="text-gradient" style={{ filter: "drop-shadow(0 0 30px rgba(255,90,0,0.3))" }}>
                {nameText}
                <motion.span animate={{ opacity: [1, 0] }} transition={{ duration: 0.5, repeat: Infinity, repeatType: "reverse" }} className={nameDone ? "hidden" : "inline-block w-[3px] h-12 bg-primary ml-1 align-middle"} />
              </span>
            </motion.h1>

            <motion.div initial={{ opacity: 0 }} animate={{ opacity: greetingDone ? 1 : 0 }} transition={{ duration: 0.3 }}>
              <motion.span className="inline-block px-4 py-2 rounded-full bg-secondary text-foreground text-sm md:text-base font-medium mb-6 border border-primary/20" whileHover={{ scale: 1.05, boxShadow: "0 0 20px rgba(255,90,0,0.3)" }} style={{ boxShadow: "0 0 10px rgba(255,90,0,0.1)" }}>
                {roleText}
                <motion.span animate={{ opacity: [1, 0] }} transition={{ duration: 0.5, repeat: Infinity, repeatType: "reverse" }} className={roleText.length >= 31 ? "hidden" : "inline-block w-[2px] h-4 bg-primary ml-1 align-middle"} />
              </motion.span>
            </motion.div>

            <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }} className="text-base md:text-lg text-muted-foreground max-w-lg mb-8 leading-relaxed">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="flex flex-col sm:flex-row items-center md:items-start gap-4">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button size="lg" className="bg-gradient-primary text-primary-foreground hover:opacity-90 px-8 relative overflow-hidden" style={{ boxShadow: "0 0 25px rgba(255,90,0,0.4), 0 0 50px rgba(255,90,0,0.1)" }}>
                  <motion.div className="absolute inset-0 bg-white/10" animate={{ x: ["-100%", "200%"] }} transition={{ duration: 2, repeat: Infinity, ease: "linear", repeatDelay: 1 }} style={{ skewX: "-20deg" }} />
                  <a href="#works">View My Work</a>
                </Button>
              </motion.div>
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button variant="outline" size="lg" className="border-primary/40 hover:border-primary hover:shadow-glow transition-all duration-300">
                  <a href="#contact">Hire Me</a>
                </Button>
              </motion.div>
            </motion.div>

            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className="flex gap-8 mt-10 justify-center md:justify-start">
              {[{ num: "2+", label: "Years Exp." }, { num: "30+", label: "Projects" }, { num: "10+", label: "Clients" }].map((stat, i) => (
                <motion.div key={i} whileHover={{ y: -4 }} className="text-center md:text-left">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8, x: 50 }} animate={{ opacity: 1, scale: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.3, type: "spring" }} className="order-1 md:order-2 flex justify-center">
            <div className="relative">
              ...
              <motion.div className="absolute -inset-4 rounded-full border-2 border-primary/20" animate={{ rotate: 360 }} transition={{ duration: 15, repeat: Infinity, ease: "linear" }} style={{ borderStyle: "dashed" }} />
              <motion.div className="absolute -inset-8 rounded-full border border-primary/10" animate={{ rotate: -360 }} transition={{ duration: 25, repeat: Infinity, ease: "linear" }} />
              <motion.div className="absolute -inset-2 rounded-full border border-primary/30" animate={{ scale: [1, 1.05, 1], opacity: [0.3, 0.6, 0.3] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} />
              <motion.div className="absolute w-3 h-3 rounded-full bg-primary" style={{ top: "50%", left: "50%", translateX: "-50%", translateY: "-50%", boxShadow: "0 0 10px rgba(255,90,0,0.8)" }} animate={{ rotate: 360, x: [0, 160, 0, -160, 0], y: [-160, 0, 160, 0, -160] }} transition={{ duration: 6, repeat: Infinity, ease: "linear" }} />

              <motion.div className="w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 rounded-full overflow-hidden bg-gradient-primary p-1" whileHover={{ scale: 1.04 }} transition={{ type: "spring", stiffness: 300 }} style={{ boxShadow: "0 0 40px rgba(255,90,0,0.2), 0 0 80px rgba(255,90,0,0.05)" }}>
                <div className="w-full h-full rounded-full bg-secondary flex items-center justify-center overflow-hidden">
                  <img src={sudhanshuPhoto} alt="Sudhanshu Khande" className="w-full h-full object-cover object-[center_40%] scale-110" />
                </div>
              </motion.div>

              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 1 }} whileHover={{ scale: 1.08, boxShadow: "0 0 20px rgba(255,90,0,0.2)" }} className="absolute -right-6 top-1/4 px-4 py-2 bg-card border border-border rounded-lg cursor-default" style={{ animation: "float 4s ease-in-out infinite", boxShadow: "0 4px 20px rgba(0,0,0,0.3)" }}>
                <span className="text-sm font-medium">⚡ 2+ Years Experience</span>
              </motion.div>

              <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 1.1 }} whileHover={{ scale: 1.08, boxShadow: "0 0 20px rgba(255,90,0,0.2)" }} className="absolute -left-6 bottom-1/4 px-4 py-2 bg-card border border-border rounded-lg cursor-default" style={{ animation: "float 4s ease-in-out 1s infinite", boxShadow: "0 4px 20px rgba(0,0,0,0.3)" }}>
